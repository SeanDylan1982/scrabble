...
interface Tile { id: string; letter: string; points: number; isBlank?: boolean }
interface BoardTile { row: number; col: number; tile: Tile }
interface PlayerState { id: string; name: string; score: number; rack: Tile[] }
interface GameState {
  roomId: string;
...
function generateTileBag(): Tile[] {
  ...
  Object.entries(LETTER_DISTRIBUTION).forEach(([letter, { count, points }]) => {
    for (let i = 0; i < count; i++) tiles.push({ id: `t${id++}`, letter, points, isBlank: letter==='?' });
  });
  return shuffle(tiles);
}

function shuffle<T>(arr: T[]): T[] { const a=[...arr]; for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1)); [a[i],a[j]]=[a[j],a[i]];} return a }
function draw(tileBag: Tile[], n: number): Tile[] { return tileBag.splice(0, Math.min(n, tileBag.length)); }

export function initGameForRoom(room: LobbyRoom) {
  // Require exactly 2 players
  if (room.players.length !== 2) throw new Error('Game requires exactly 2 players');

  const bag = generateTileBag();
  const players: PlayerState[] = room.players.map(p => ({ id: p.id, name: p.name, score: 0, rack: draw(bag, 7) }));
...
function getLetterPoints(letter: string) { return LETTER_DISTRIBUTION[letter.toUpperCase()]?.points || 0 }

function calculateWordScore(word: string, positions: Array<{row:number;col:number}>, newPositions: Array<{row:number;col:number}>) {
  let score = 0; let wordMult = 1;
  positions.forEach(({row,col}, i) => {
    ...
    if (isNew) {
      ...
      if (premium==='DL') letterMult=2; else if (premium==='TL') letterMult=3; else if (premium==='DW') wordMult*=2; else if (premium==='TW') wordMult*=3;
    }
...
function extendWord(board: (BoardTile|null)[][], row:number, col:number, dir:'h'|'v') {
  let r=row, c=col;
  while (r>0 && (dir==='v' ? board[r-1][c] : board[r][c-1])) { if (dir==='v') r--; else c--; }
  const positions: {row:number; col:number}[] = [];
  const letters: string[] = [];
  while (r<15 && c<15 && board[r][c]) { positions.push({row:r,col:c}); letters.push(board[r][c]!.tile.letter); if (dir==='v') r++; else c++; }
...
export const getGameStateHandler: RequestHandler = (req, res) => {
  ...
  let game: GameState;
  try { game = getGameOr404(roomId); } catch (e:any) { return res.status(e.code||500).json({ error: e.message }); }

  const redactedPlayers = game.players.map(p => ({
    id: p.id, name: p.name, score: p.score, rack: p.id===playerId ? p.rack : [] as Tile[], rackCount: p.rack.length
  }));
...
export const playMoveHandler: RequestHandler = (req, res) => {
  const roomId = req.params.id; const { playerId, placements } = req.body as { playerId: string, placements: Array<{row:number; col:number; tileId:string; letter?:string}> };
  let game: GameState; try { game = getGameOr404(roomId); } catch (e:any) { return res.status(e.code||500).json({ error: e.message }); }
  if (game.currentPlayerId !== playerId) return res.status(400).json({ error: 'Not your turn' });
  if (!placements || placements.length===0) return res.status(400).json({ error: 'No tiles placed' });

  const player = game.players.find(p=>p.id===playerId)!;
  // Validate tiles belong to player
  const rackIds = new Set(player.rack.map(t=>t.id));
  for (const pl of placements) if (!rackIds.has(pl.tileId)) return res.status(400).json({ error: 'Invalid tile' });

  const sameRow = placements.every(p=>p.row===placements[0].row);
  const sameCol = placements.every(p=>p.col===placements[0].col);
  if (!sameRow && !sameCol) return res.status(400).json({ error: 'Tiles must be in one row or column' });

  // No overwrite existing tiles
  for (const p of placements) if (game.board[p.row][p.col]) return res.status(400).json({ error: 'Square occupied' });

  // First move must cover center
  const isFirstMove = game.turnNumber===1 && game.players.every(pl=>pl.score===0) && game.board.flat().every(c=>c===null);
  if (isFirstMove && !placements.some(p=>p.row===7 && p.col===7)) return res.status(400).json({ error: 'First word must cover center' });
  ...
  for (let r=minR; r<=maxR; r++) for (let c=minC; c<=maxC; c++) {
    if ((dir==='h' && r===placements[0].row) || (dir==='v' && c===placements[0].col)) {
      if (!game.board[r][c] && !placements.find(p=>p.row===r && p.col===c)) return res.status(400).json({ error: 'Tiles must be contiguous' });
    }
  }
  ...
  for (const p of placements) {
    ...
    const letter = rackTile.isBlank && p.letter ? p.letter.toUpperCase() : rackTile.letter;
    tempBoard[p.row][p.col] = { row: p.row, col: p.col, tile: { ...rackTile, letter } };
    newPositions.push({ row: p.row, col: p.col });
  }

  // Find all words
  const main = dir==='h' ? extendWord(tempBoard, placements[0].row, Math.min(...placements.map(p=>p.col)), 'h') : extendWord(tempBoard, Math.min(...placements.map(p=>p.row)), placements[0].col, 'v');
  const words: { word:string; positions:{row:number;col:number}[] }[] = [];
  if (main.word.length>=2) words.push(main);
  for (const p of placements) {
    const cross = dir==='h' ? extendWord(tempBoard, p.row, p.col, 'v') : extendWord(tempBoard, p.row, p.col, 'h');
    if (cross.word.length>=2) words.push(cross);
  }
  if (words.length===0) return res.status(400).json({ error: 'No valid word formed' });

  // Score
  let totalScore = 0;
  for (const w of words) totalScore += calculateWordScore(w.word, w.positions, newPositions);
  ...
  for (const p of placements) game.board[p.row][p.col] = tempBoard[p.row][p.col];

  // Draw
  const newTiles = draw(game.tileBag, Math.min(7 - player.rack.length, game.tileBag.length));
...
export const passTurnHandler: RequestHandler = (req, res) => {
  const roomId = req.params.id; const { playerId } = req.body as { playerId: string };
  let game: GameState; try { game = getGameOr404(roomId); } catch (e:any) { return res.status(e.code||500).json({ error: e.message }); }
  if (game.currentPlayerId !== playerId) return res.status(400).json({ error: 'Not your turn' });
  const other = game.players.find(p=>p.id!==playerId)!;
  game.currentPlayerId = other.id; game.turnNumber += 1;
  res.json({ ok: true });
};
