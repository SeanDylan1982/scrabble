  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/lobby/rooms', {
        ...
        body: JSON.stringify({ name: roomName, playerName } as CreateRoomRequest)
      });
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            <Button onClick={soloStart} className="bg-green-600 hover:bg-green-700 gap-2">
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                ...
                <Input value={roomName} onChange={e => setRoomName(e.target.value)} placeholder="e.g. Friday Night" />
              </div>
              <Button onClick={handleCreate} disabled={!playerName || creating} className="w-full gap-2">
                <Plus className="w-4 h-4" /> Create
              </Button>
              <p className="text-xs text-gray-500">Share the room link with friends after creating.</p>
            </CardContent>
          </Card>
                  ...
                  {rooms.map(room => (
                    <div key={room.id} className="border rounded-lg p-4 bg-white shadow-sm">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-semibold text-gray-800 flex items-center gap-2">
                            {room.name}
                            {room.status === 'started' && (
                              <span className="text-xs px-2 py-0.5 bg-amber-100 text-amber-800 rounded-full">In game</span>
                            )}
                          </div>
                        ...
                        <div className="flex gap-2">
                          {room.status === 'waiting' ? (
                            <Button size="sm" onClick={() => handleJoin(room.id)} className="gap-2">
                      ...
                      <div className="flex items-center gap-2 mt-3 flex-wrap">
                        {room.players.map(p => (
                          <span key={p.id} className="px-2 py-1 text-xs rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                            {p.isHost && <Crown className="w-3 h-3 text-yellow-500" />} {p.name}
                          </span>
                        ))}
                      </div>
                    </div>
