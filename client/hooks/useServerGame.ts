...
export interface ServerBoardTile {
  row: number;
  col: number;
  tile: ServerTile;
}
...
export function useServerGame(roomId: string) {
  ...
  const [placed, setPlaced] = useState<
    { row: number; col: number; tile: ServerTile }[]
  >([]);
  ...
  const deselectTile = (tileId: string) =>
    setSelected((prev) => prev.filter((t) => t.id !== tileId));
  ...
  const recallTiles = () => {
    setSelected((prev) => [...prev, ...placed.map((p) => p.tile)]);
    setPlaced([]);
  };
  ...
  const passTurn = async () => {
    const res = await fetch(`/api/game/${roomId}/pass`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return {
    ...
    actions: {
      selectTile,
      deselectTile,
      placeTile: (r: number, c: number) => placeTile(r, c),
      removeTile,
      playWord,
      shuffleRack,
      recallTiles,
